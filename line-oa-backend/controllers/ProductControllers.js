...
const createProduct = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: "Invalid ingredients format", details: err.message });
    }
  ...
  try {
    ...
    try {
      ...
      return res.status(400).json({
        error: "Invalid JSON format in ingredients",
        details: err.message,
      });
    }
...
const generateFlexMenu = (products) => {
  const contents = products.map((product) => ({
    ...
    hero: {
      type: "image",
      url: `https://8140-2405-9800-b520-60b1-fdfc-f7d3-36ac-7eff.ngrok-free.app/uploads/${product.Product_img}`,
      size: "full",
      aspectRatio: "20:13",
      aspectMode: "cover",
    },
